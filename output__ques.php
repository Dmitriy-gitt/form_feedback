...
if($_GET['id']){
    ...
    $topic = $_GET['id'];
    function call_sum_records($object, $get, $welcome)
    {
        //$result = mysqli_query($db, "SELECT COUNT(*) FROM answers WHERE id_ques IN (SELECT id FROM questions WHERE topic = 2)");
        // Определяем общее число сообщений в базе данных
        return $object->get_sum_records($get, $welcome);
    }

    $temp = call_sum_records($questions_obj ,$_GET['id'], $welcome);
}

include_once 'pagination/nav.php';
...
<?php 
    ...
    do {
        $ques = $questions_obj->get($welcome, $topic, $start, $num);
        
        if (is_array($ques) || $ques instanceof Countable) {
            for($i=0; $i<count($ques); $i++){
                $answers = $answers_obj->get($ques[$i]['id']);
                echo"
            <div class='ques'>Автор: ".$ques[$i]['author']."<br> Пользователь: ".$ques[$i]['visitor']."<br><p style='border-bottom: 1px solid black;'>".$ques[$i]['ques']."</p>";
...
<?php 
require_once 'pagination/pagUser.php';
?>
</body>
</html>